function [ warpImg, gridIntensities,H,dp ] = warpRectangle( img, rect, gridX, gridY, maxOffset )
...
dp = [transA;transB;transC;transD];
...
% warpImg = warpImg+normrnd(0,0.1,newRows,newCols);
    ...
    gridIntensities = getGridIntensities(img,gridX+maxOffset,gridY+maxOffset,invH);

    
    s = size(gridX,2);
    
    %normalize intensity values (mean = 0, standard deviation = 1)
    gridIntensities = normalizeMatrix(gridIntensities);

    %add some random noise to intensity values
    gridIntensities = gridIntensities+normrnd(0,0.1,s,s);
